...
const isLabelsSupported = (servers) =>
    servers.some((server) => {
        ...
        if (
            ...
            client.clientCapabilities.includes("label")
        ) {
            return true;
        }
        return false;
...
const persistOptions = () => {
    ...
    const directories =
        document.querySelector("#directories").value.split(/\n/g) || [];
    ...
    const client = clientList.find(
        (client) => client.id === document.querySelector("#application").value
    );

    let httpAuth = null;
    if (
        document.querySelector("#httpAuth").checked &&
        client &&
        client.clientCapabilities &&
        !client.clientCapabilities.includes("httpAuth")
    ) {
        httpAuth = {
    ...
    options.servers[~~serverSelect.value] = {
        ...
        directories: directories
            ...
            .filter((directory) => directory.length),
...
const restoreOptions = () => {
    ...
    document.querySelector("#matchregexp").placeholder =
        "/^https:\\/\\/example\\.com\\/torrent\\/\\d+\\/download\\/$/".replace(
            /\\n/g,
            "\n"
        );
    document.querySelector("#labels").placeholder =
        "Label\nAnother label".replace(/\\n/g, "\n");
    document.querySelector("#directories").placeholder =
    ...
    loadOptions().then((newOptions) => {
        ...
        document.querySelector("#matchregexp").value =
            options.globals.matchRegExp.join("\n");
        document.querySelector("#matchregexp").disabled =
            options.globals.catchUrls === false;
...
const getRegExps = () => {
    const matchRegExp =
        document.querySelector("#matchregexp").value.split(/\n/g) || [];
    return matchRegExp
...
document.querySelector("#httpAuth").addEventListener("change", (e) => {
    document.querySelector("#httpAuthUsername").disabled =
        !e.currentTarget.checked;
    document.querySelector("#httpAuthPassword").disabled =
        !e.currentTarget.checked;
});
document.querySelector("#test-regexp").addEventListener("click", (e) => {
    ...
    const matchingRegExp =
        regExps.find((regExp) => regExp.exec(testUrl) !== null) || null;

    document.querySelector("#test-regexp-result").textContent = matchingRegExp
        ? chrome.i18n.getMessage("testSuccessText", matchingRegExp.toString())
        : chrome.i18n.getMessage("testFailText");
});
...
document.querySelector("#application").addEventListener("change", (e) => {
    ...
    if (client) {
        ...
        document.querySelector('[data-panel="directories"]').style.display =
            client.clientCapabilities && client.clientCapabilities.includes("path")
                ? "flex"
                : "none";

        document.querySelector('[data-panel="labels"]').style.display =
            isLabelsSupported(options.servers) ||
                (client.clientCapabilities && client.clientCapabilities.includes("label"))
                ? "flex"
                : "none";
